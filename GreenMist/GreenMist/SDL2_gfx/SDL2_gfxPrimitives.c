...
{
	return SDL_RenderDrawLine(renderer, (int)x1, (int)y, (int)x2, (int) y);
}
	...
	do {
		...
		if (ocy != cy) {
			if (cy > 0) {
				ypcy = y + cy;
				ymcy = y - cy;
				result |= _hline(renderer, xmcx, xpcx, ypcy);
				result |= _hline(renderer, xmcx, xpcx, ymcy);
			} else {
				result |= _hline(renderer, xmcx, xpcx, y);
			}
			ocy = cy;
		}
		if (ocx != cx) {
			if (cx != cy) {
				if (cx > 0) {
					ypcx = y + cx;
					ymcx = y - cx;
					result |= _hline(renderer, xmcy, xpcy, ymcx);
					result |= _hline(renderer, xmcy, xpcy, ypcx);
				} else {
					result |= _hline(renderer, xmcy, xpcy, y);
				}
			}
			ocx = cx;
		}
	...
	if (rx > ry) {
		...
		do {
			...
			if ((ok != k) && (oj != k)) {
				...
				xmh = x - h;
				if (k > 0) {
					result |= _hline(renderer, xmh, xph, y + k);
					result |= _hline(renderer, xmh, xph, y - k);
				} else {
					result |= _hline(renderer, xmh, xph, y);
				}
				ok = k;
			}
			if ((oj != j) && (ok != j) && (k != j)) {
				...
				xpi = x + i;
				if (j > 0) {
					result |= _hline(renderer, xmi, xpi, y + j);
					result |= _hline(renderer, xmi, xpi, y - j);
				} else {
					result |= _hline(renderer, xmi, xpi, y);
				}
				oj = j;
			}
		...
		do {
			...
			if ((oi != i) && (oh != i)) {
				...
				xpj = x + j;
				if (i > 0) {
					result |= _hline(renderer, xmj, xpj, y + i);
					result |= _hline(renderer, xmj, xpj, y - i);
				} else {
					result |= _hline(renderer, xmj, xpj, y);
				}
				oi = i;
			}
			if ((oh != h) && (oi != h) && (i != h)) {
				...
				xpk = x + k;
				if (h > 0) {
					result |= _hline(renderer, xmk, xpk, y + h);
					result |= _hline(renderer, xmk, xpk, y - h);
				} else {
					result |= _hline(renderer, xmk, xpk, y);
				}
				oh = h;
			}
